...
#                TEMP_LANES[$LANE_LEN] = $CUR_LANE
    ...
    sed -i '/^U/ d' ${OUT_DIR}${TEMP_NAME}.kraken.output.tsv
    DB="DBPathogen"
    bash ./scripts/run_Kraken.sh -k $KRAKEN_DIR -b "$DB" -i "--fasta-input ${OUT_DIR}${TEMP_NAME}.kraken.unclassified.fasta" -o ${OUT_DIR}temp2/ -n ${TEMP_NAME} -w $WORK_DIR -d $DB_DIR -c
    echo "Filtering"
    bash ./scripts/run_Kraken_filter.sh -k $KRAKEN_DIR -b "$DB" -o ${OUT_DIR}temp2/ -n ${TEMP_NAME} -w $WORK_DIR -v $FILTER -d $DB_DIR
    echo "Concatenating 2"
    cat `find ${OUT_DIR}temp2/ -name '*.output.filter.tsv' -print` >> ${OUT_DIR}${TEMP_NAME}.kraken.output.tsv
    cat `find ${OUT_DIR}temp2/ -name '*.classified.fasta' -print` >> ${OUT_DIR}${TEMP_NAME}.kraken.classified.fasta
    cat `find ${OUT_DIR}temp/ -name '*.unclassified.fasta' -print` > ${OUT_DIR}${TEMP_NAME}.kraken.unclassified.fasta
